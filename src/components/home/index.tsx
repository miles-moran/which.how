...
import {poll, which_character_are_you_sample} from '../../utils/samples';

export const HomeComponent: React.FC<HomeStackNavProps<'Home'>> = ({
  navigation,
}) => {
  const [trueFalseQuiz, setTrueFalseQuiz] = useState<Quiz>();
  const [mutipleChoiceQuiz, setMultipleChoiceQuiz] = useState<Quiz>();
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        setLoading(false);
        setMultipleChoiceQuiz(parseQuiz(res));
      });
  }, []);
  ...
  if (loading || !trueFalseQuiz || !mutipleChoiceQuiz) {
    return <Loading />;
  }
  ...
  const quizzes = [
    ...
    {text: 'Multiple Choice Quiz', data: mutipleChoiceQuiz},
